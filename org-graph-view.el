    ...
    (cl-labels ((rec (tree)
                ...
                (path-id (path)
                         (or (gethash path nodes)
                             (puthash path (format "node:%s" (cl-incf node-id)) nodes)))
                (node-id (node path)
                         (-let* (((_element (&plist :raw-value) . children) node))
                           (or (gethash path nodes)
                               (puthash path (format "node:%s" (cl-incf node-id)) nodes))))
                (format-tree (tree &optional path)
                             (--map (format-node it path)
                                    (cddr tree)))
                (format-node (node &optional path)
                             (-let (((_element (&plist :raw-value) . children) node))
                               (if children
                                   (--map (concat (path-id (append path (list raw-value))) " -> " it)
                                          (-flatten (format-tree node (append path (list raw-value)))))
                                 (progn
                                   (concat (path-id (append path (list raw-value))) ";\n")))))
                )
      (org-with-wide-buffer
       (when (org-at-heading-p)
         (org-narrow-to-subtree))

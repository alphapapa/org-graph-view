  ...
  (let* ((node-id 0)
    ...
    (cl-labels ((format-tree (tree &optional path)
                ...
                (format-node (node &optional path)
                             (-let (((_element (&plist :raw-value) . children) node))
                               (if children
                                   (--map (concat (node-id node path) " -> " it)
                                          (-flatten (format-tree node (append path (list raw-value)))))
                                 (progn
                                   (concat (node-id node path) ";\n")))))
                (node-id (node path)
                         (-let* (((_element (&plist :raw-value) . children) node)
                                 (key (append path (list raw-value))))
                           (or (car (gethash key nodes))
                               (let ((node-id (format "node%s" (cl-incf node-id))))
                                 (puthash key (cons node-id node)
                                          nodes)
                                 node-id))))
